    ...
    def set_geo_location(self, loc, switch_back=True):
        ...
        if self.ios:
            ...
            try:
                ...
                raise RuntimeError("In set_geo_location(). To use this method, you must install the Fake Location app and put its icon on your Android Home Screen")

            self.tap(["Stop the service", "Stop"])   # diff versions of Fake Location have diff button names
            self.sleep(8)
            self.tap("Favorites")
            self.tap(loc.name)
            self.sleep(1.5)
